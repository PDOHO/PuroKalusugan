        ...
        <div className="px-6 py-4 border-t border-charcoal-gray/10 flex flex-col md:flex-row items-center justify-between gap-4 bg-white">
          <div className="flex items-center gap-4">
            <div className="text-sm text-blue-slate">
              {(() => {
                const hasFilters = Boolean(searchTerm || filterMunicipality || filterBarangay || filterProgram || filterYear || filterMonth || filterLargeScale || showNewOnly);
                return `Showing ${total === 0 ? 0 : ((page - 1) * limit) + 1} to ${Math.min(page * limit, total)} of ${hasFilters ? total.toLocaleString() : '~' + total.toLocaleString() + ' entries (estimated)'}${hasFilters ? ' entries' : ''}`;
              })()}
            </div>
            <select
              value={limit}
