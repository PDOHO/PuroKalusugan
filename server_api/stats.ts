...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (refresh === "true") {
      cache.del(cacheKey);
      await supabase.from('api_cache').delete().eq('key', cacheKey);
      console.log(
        `[Stats API] Cache key deleted for: ${cacheKey} (forced refresh)`,
      );
    } else {
      // 1. Check in-memory node-cache first (fastest)
      const memCachedData = cache.get(cacheKey);
      if (memCachedData) {
        console.log(`[Stats API] Memory cache hit for key: ${cacheKey}`);
        return res.json(memCachedData);
      }
      
      // 2. Check Supabase persistent cache table (api_cache)
      try {
        ...
        if (!cacheErr && dbCache) {
          const cacheAgeMs = new Date().getTime() - new Date(dbCache.updated_at).getTime();
          // If the cache is less than 4 hours old (14400000 ms), use it!
          if (cacheAgeMs < 14400000) {
            console.log(`[Stats API] DB cache hit for key: ${cacheKey}`);
            // Save to memory cache for subsequent requests in this container
            cache.set(cacheKey, dbCache.data);
            
            res.setHeader("Cache-Control", "s-maxage=300, stale-while-revalidate=59");
            return res.json(dbCache.data);
          }
        }
      } catch (checkErr) {
        // Table might not exist yet, just continue to run the normal query
        console.log(`[Stats API] DB cache check skipped (table might not exist yet)`);
      }
    }

    // --- HEAVY DB QUERIES START HERE ---
    ...
    try {
      ...
      if (!rpcError && rpcData) {
        ...
        // Save to Supabase persistent cache (fire and forget intentionally)
        supabase.from('api_cache').upsert({
          ...
          if (error) {
            if (error.message && error.message.includes('row-level security')) {
              console.log(`[Stats API] DB cache upsert skipped (RLS policy missing for api_cache)`);
            } else {
              console.log(`[Stats API] DB cache upsert failed: ${error.message}`);
            }
          } else {
            console.log(`[Stats API] DB cache updated for: ${cacheKey}`);
          }
        });
