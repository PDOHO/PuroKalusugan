...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    if (req.method === 'GET') {
      ...
      const hasFilters = Boolean(search || municipality || barangay || program || year || month || large_scale || new_only);
      ...
      const getCountQuery = () => {
        let q;
        const countType = hasFilters ? 'exact' : 'estimated';
        if (hasServiceFilter) {
          ...
          q = supabase.from('patients').select(`id, patient_services!inner(${subfields.join(',')})`, { count: countType, head: true });
        } else {
          q = supabase.from('patients').select('id', { count: countType, head: true });
        }
